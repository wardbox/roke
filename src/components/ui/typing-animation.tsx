...
import { typingCursor, typingText } from "./motion";

const DEFAULT_TEXTS = [
  "Check out this typed text!",
];
  ...
  useEffect(() => {
    ...
    const typeNextChar = async () => {
      ...
      if (charIndex < currentText.length) {
        ...
        const randomDelay = Math.random() * (typingSpeed.max - typingSpeed.min) + typingSpeed.min;
  ...
  return (
    <span className={className}>
      ...
      <motion.div
        ...
        animate="blinking"
        className="inline-block h-[1em] w-[0.1em] translate-y-[0.1em] bg-current ml-[0.1em]"
      />
    </span>
  );
} 
